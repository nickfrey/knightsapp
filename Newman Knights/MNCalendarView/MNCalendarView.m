...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if ([cell isKindOfClass:MNCalendarViewDayCell.class] && cell.enabled) {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(calendarView:didSelectDate:atIndexPath:)]) {
      [self.delegate calendarView:self didSelectDate:dayCell.date atIndexPath:indexPath];
    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  CGFloat itemHeight = indexPath.item < self.daysInWeek ? 30.f : (landscape ? itemWidth-35 : itemWidth);
